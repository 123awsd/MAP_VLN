...
A coarse 3D A* query that shares occupancy semantics with the execution layer estimates candidate-to-candidate motion costs; unreachable transitions receive infinite cost rather than Euclidean fallback. The planner jointly selects a topologically valid task order and representative observation poses. Final execution is authorized by full-resolution 3D A*, collision checks, and B-spline optimization. Cached motion costs are tied to the geometry-map version.
