...
[[ "${REALSENSE_RGB_AUTO_EXPOSURE:-}" == "false" ]] || {
  echo "This capture helper currently requires manual RGB exposure." >&2
  exit 2
}
[[ "${REALSENSE_RGB_EXPOSURE:-}" =~ ^[0-9]+$ ]] || {
...
rosrun dynamic_reconfigure dynparam set "$RGB_NODE" enable_auto_exposure false >/dev/null
...
grep -Eq "enable_auto_exposure['\"]?: (false|False)" <<<"$actual" || {
  echo "RealSense rejected manual RGB exposure mode." >&2
  exit 1
}
