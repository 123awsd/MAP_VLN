...
case "$mode" in
  ...
  *) echo "Usage: $0 [reliable|all]" >&2; exit 2 ;;
esac

if [[ -f "$data/scene9002_00_calib01/manifest.json" ]]; then
...
for path in "$data/fastlio_complete/handheld_map_Drone_room_complete.pcd" \
            "$localization/target_points_clustered.json" \
            "$localization/target_points_raw.json" \
            "$boxer_scene/boxer_3dbbs.csv" \
            "$data/fastlio_complete/trajectory.csv" \
            "$mission"; do
...
exec "${docker_cmd[@]}" run --rm --init --net host \
  ...
  "$image" bash -lc '
    ...
    python3 /workspace/project/real_fly/stage2_offline/rviz/publish_drone_room_visualization.py \
      /workspace/project/real_fly/stage2_offline/data/Drone_room/fastlio_complete/handheld_map_Drone_room_complete.pcd \
      ...
      /workspace/project/real_fly/stage2_offline/data/Drone_room/fastlio_complete/trajectory.csv \
