...
/*=====================================================*/
    ...
    GridType InfMap::getGridType(const Vec3f& pos) const {
        Vec3i id_g, id_l;
        // 1. check virtual ceil and ground
        // Config::resetMapSize() has already quantized virtual bounds onto the
        // inflated-grid boundary (including inflation_step).  Applying another
        // (1 + inflation_step) cells here would inflate the virtual ground and
        // ceiling twice: e.g. a 0.25 m effective ground becomes 0.65 m with a
        // 0.10 m grid and step 3.  Keep InfMap consistent with ProbMap and use
        // the effective bounds exactly once.
