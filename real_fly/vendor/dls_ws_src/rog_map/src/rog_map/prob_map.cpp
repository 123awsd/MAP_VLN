...
void ProbMap::updateProbMap(const PointCloud& cloud, const Pose& pose) {
    ...
    if (pos.z() > cfg_.virtual_ceil_height) {
        ...
        if (now - last_virtual_ground_warning_ >= std::chrono::seconds(5)) {
            std::cout << YELLOW << " -- [ROGMapCore] Odom below virtual ground; map update is deferred until odometry rises above "
                      << cfg_.virtual_ground_height << " m. -- " << RESET << std::endl;
            last_virtual_ground_warning_ = now;
        }
        return;
    }
