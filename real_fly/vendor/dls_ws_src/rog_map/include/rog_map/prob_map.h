...
namespace rog_map {
    ...
    protected:
        ...
        // This is deliberately per-map-instance.  A recorder preview may
        // replace its immutable PCD and rebuild a fresh ROG-Map in the same
        // process; a function-local static here incorrectly rejected that.
        bool prob_map_initialized_{false};
        // Invalid initial odometry can remain below virtual ground until the
        // aircraft takes off.  Keep rejecting those clouds, but avoid printing
        // the same non-actionable warning for every LiDAR frame.
        std::chrono::steady_clock::time_point last_virtual_ground_warning_{};
        struct RaycastData {
