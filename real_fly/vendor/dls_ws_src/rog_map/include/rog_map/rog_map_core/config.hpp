...
#define CMAKE_ROOT_DIR(name) (string(string(ROOT_DIR) + name))
    ...
    public:
        ...
        Config(const string &cfg_path, const string &name_space = "rog_map") {
            ...
            loader.LoadParam(name_space + "/ros_callback/occupied_cloud_topic", occupied_cloud_topic, string(""));
            ...
            virtual_ground_height_raw = virtual_ground_height;
            virtual_ceil_height_raw = virtual_ceil_height;
...
#define logit(x) (log((x) / (1 - (x))))
         ...
         */
        // Keep the configured physical planes as well as the quantized,
        // inflation-layer bounds produced by resetMapSize().  A* consumes the
        // latter; a geometric corridor builder needs the former and applies
        // its robot radius exactly once.
        double virtual_ceil_height{}, virtual_ground_height{};
        double virtual_ceil_height_raw{}, virtual_ground_height_raw{};
        ...
        string odom_topic{}, cloud_topic{}, occupied_cloud_topic{};
        /* probability update */
