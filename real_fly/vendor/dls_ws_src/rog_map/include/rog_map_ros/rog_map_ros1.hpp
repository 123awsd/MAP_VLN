...
namespace rog_map {
    ...
    class ROGMapROS :public ROGMap {
        ...
        struct ROSCallback {
            ros::Subscriber odom_sub, cloud_sub, occupied_cloud_sub;
            int unfinished_frame_cnt{0};
            Pose pc_pose;
            PointCloud pc;
            PointCloud::Ptr occupied_pc;
        ...
        void occupiedCloudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
            PointCloud::Ptr tmp_pc(new PointCloud);
            pcl::fromROSMsg(*cloud_msg, *tmp_pc);
            std::lock_guard<std::mutex> lock(rc_.updete_lock);
            rc_.occupied_pc = std::move(tmp_pc);
        }
        ...
        void updateCallback(const ros::TimerEvent& event) {
            ...
            {
                std::lock_guard<std::mutex> lock(rc_.updete_lock);
                occupied_pc = rc_.occupied_pc;
            }
            if (map_empty_ && occupied_pc && !occupied_pc->empty()) {
                // Static constraints and prior-map points are occupied-only observations.
                // They must not generate synthetic free-space rays through real obstacles.
                updateOccPointCloud(*occupied_pc);
                map_empty_ = false;
            }
            ...
            // Re-apply immutable constraints after free-space raycasting so live
            // lidar misses can clear dynamic trails but never erase virtual ceilings.
            if (occupied_pc && !occupied_pc->empty()) {
                updateOccPointCloud(*occupied_pc);
            }
    ...
    public:
        ...
        ROGMapROS(const ros::NodeHandle& nh, const std::string& cfg_path,
                  const bool enable_ros_callbacks = true,
                  const bool enable_visualization = true) :nh_(nh){
            cfg_ = rog_map::Config(cfg_path);
            // Offline full-smooth validation inserts an immutable PCD through
            // updateOccPointCloud().  It must not create a second live map
            // pipeline or visualization timer merely because the shared SUPER
            // configuration enables them for the flight planner.
            cfg_.ros_callback_en = cfg_.ros_callback_en && enable_ros_callbacks;
            cfg_.visualization_en = cfg_.visualization_en && enable_visualization;
            init();
            ...
            if (cfg_.ros_callback_en) {
                ...
                if (!cfg_.occupied_cloud_topic.empty()) {
                    rc_.occupied_cloud_sub = nh_.subscribe(cfg_.occupied_cloud_topic, 1,
                                                           &ROGMapROS::occupiedCloudCallback, this);
                }
