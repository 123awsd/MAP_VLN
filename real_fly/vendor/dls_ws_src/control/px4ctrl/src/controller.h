...
struct Desired_State_t
{
	...
	Desired_State_t(Odom_Data_t &odom)
		: p(odom.p),
		  ...
		  q(odom.q),
		  dir(Eigen::Vector3d(cos(yaw), sin(yaw), 0)),
		  ddir(Eigen::Vector3d::Zero()),
		  yaw_rate(0){};
};
...
private:
	...
	static constexpr double kMaxAngularAcc_ = 100;
	...
	static constexpr double dh = 0.25;
	static constexpr double dv = 0.35;
	static constexpr double cp = 0.01;
	static constexpr double veps = 0.02; // ms^-s
};
