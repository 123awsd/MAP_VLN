...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle &nh)
{
	...
	read_essential_param(nh, "use_yaw_recal", use_yaw_recal);
	...
	nh.param("auto_takeoff_land/takeoff_speed", takeoff_land.takeoff_speed, takeoff_land.speed);
	nh.param("auto_takeoff_land/land_speed", takeoff_land.land_speed, takeoff_land.speed);
	nh.param("auto_takeoff_land/motors_speedup_time", takeoff_land.motors_speedup_time, 3.0);
	nh.param("auto_takeoff_land/post_takeoff_delay", takeoff_land.post_takeoff_delay, 2.0);
	nh.param("auto_takeoff_land/land_detect_position_error",
			 takeoff_land.land_detect_position_error, 0.5);
	nh.param("auto_takeoff_land/land_detect_velocity_threshold",
			 takeoff_land.land_detect_velocity_threshold, 0.1);
	nh.param("auto_takeoff_land/land_detect_keep_time", takeoff_land.land_detect_keep_time, 3.0);
	nh.param("auto_takeoff_land/disarm_retry_interval", takeoff_land.disarm_retry_interval, 1.0);

	ROS_ASSERT_MSG(takeoff_land.takeoff_speed > 0.0, "takeoff_speed must be positive");
	ROS_ASSERT_MSG(takeoff_land.land_speed > 0.0, "land_speed must be positive");
	ROS_ASSERT_MSG(takeoff_land.motors_speedup_time >= 0.0, "motors_speedup_time must be non-negative");
	ROS_ASSERT_MSG(takeoff_land.post_takeoff_delay >= 0.0, "post_takeoff_delay must be non-negative");
	ROS_ASSERT_MSG(takeoff_land.land_detect_position_error > 0.0,
			   "land_detect_position_error must be positive");
	ROS_ASSERT_MSG(takeoff_land.land_detect_velocity_threshold > 0.0,
			   "land_detect_velocity_threshold must be positive");
	ROS_ASSERT_MSG(takeoff_land.land_detect_keep_time >= 0.0,
			   "land_detect_keep_time must be non-negative");
	ROS_ASSERT_MSG(takeoff_land.disarm_retry_interval > 0.0,
			   "disarm_retry_interval must be positive");

	ROS_INFO("[px4ctrl] auto takeoff/land: height=%.2f m, up=%.2f m/s, down=%.2f m/s, "
			 "motor_ramp=%.2f s, trigger_delay=%.2f s, "
			 "land_detect=[%.2f m, %.2f m/s, %.2f s], disarm_retry=%.2f s",
			 takeoff_land.height, takeoff_land.takeoff_speed, takeoff_land.land_speed,
			 takeoff_land.motors_speedup_time, takeoff_land.post_takeoff_delay,
			 takeoff_land.land_detect_position_error,
			 takeoff_land.land_detect_velocity_threshold,
			 takeoff_land.land_detect_keep_time,
			 takeoff_land.disarm_retry_interval);
