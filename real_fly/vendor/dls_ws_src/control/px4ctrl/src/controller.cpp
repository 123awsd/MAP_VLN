    ...
    double voltage)
{
  ...
  Eigen::Quaterniond desired_attitude, ideal_att;
  Eigen::Vector4d quat;
  Eigen::Vector3d ideal_omega;
  double thrust, ideal_thrust;

  translational_acc = (Gravity + computeLimitedTotalAccFromThrustForce(translational_acc - Gravity, 1.0)).eval();

  double yaw_ = des.yaw, yaw_rate_ = des.yaw_rate;
  // calculate ideal att thr and omg, with or W/O recal yaw
  if (param.use_yaw_recal)
  {
    flatnessWithDrag(des.v, des.a, des.j, des.dir, des.ddir, yaw_, yaw_rate_,
                     ideal_thrust, quat, ideal_omega, param.mass, param.gra);
  }
  else
  {
    flatnessWithDrag(des.v, des.a, des.j, des.yaw, des.yaw_rate,
                     ideal_thrust, quat, ideal_omega, param.mass, param.gra);
  }
  ideal_att = Eigen::Quaterniond(quat(0), quat(1), quat(2), quat(3));

  minimumSingularityFlatWithDrag(param.mass, param.gra, des.v, translational_acc, des.j,
                                 des.dir, des.ddir, odom.q, yaw_, yaw_rate_,
                                 desired_attitude, u.bodyrates, thrust);

  debug.forward_rate_x = u.bodyrates(0);
  debug.forward_rate_y = u.bodyrates(1);
  debug.forward_rate_z = u.bodyrates(2);
  ...
  Eigen::Quaterniond trans_q = odom.q.inverse() * desired_attitude;
  Eigen::Matrix3d trans_R(trans_q);
  const Eigen::Vector3d temp_bodyrates = u.bodyrates;
  u.bodyrates = trans_R * temp_bodyrates;
  debug.trans_rate_x = u.bodyrates(0);
  debug.trans_rate_y = u.bodyrates(1);
  debug.trans_rate_z = u.bodyrates(2);
  ...
  debug.ideal_thr = ideal_thrust;
  debug.ideal_rate_x = ideal_omega(0);
  debug.ideal_rate_y = ideal_omega(1);
  debug.ideal_rate_z = ideal_omega(2);
  // Compute the error quaternion wmywmy
  const Eigen::Quaterniond q_e = ideal_att.inverse() * desired_attitude;
  ...
  debug.fb_axisang_ang = rotation_vector.angle() * 180 / M_PI;
  debug.ideal_q_x = ideal_att.x();
  debug.ideal_q_y = ideal_att.y();
  debug.ideal_q_z = ideal_att.z();
  debug.ideal_q_w = ideal_att.w();
  // wmywmy debug
    ...
    const double &mass) const
{
  ...
  {
    ROS_ERROR("total_acc less than 3.0m/s^2, which may lead to a decrease in aircraft control capability!");
                                  ...
                                  const double &grav) const
{
  ...
  double qx, qy, qw, dqx, dqy, dqw;
  double x0, dx0, x1, dx1, x2, dx2;
                                                ...
                                                double &thrust) const
{
  ...
  // need to add dir singularity return false
  if (param.use_yaw_recal)
    ret = flatnessWithDrag(vel, acc, jer, dir, ddir,
                           yaw, yawd, thrust, quat, omg,
                           mass, grav);
  else
    ret = flatnessWithDrag(vel, acc, jer, yaw, yawd,
                           thrust, quat, omg,
                           mass, grav);
    ...
    const Parameter_t &param)
{
  ...
  bodyrates.x() = KAng(0) * rotation_vector.angle() * axis(0);
  bodyrates.y() = KAng(1) * rotation_vector.angle() * axis(1);
  bodyrates.z() = KAng(2) * rotation_vector.angle() * axis(2);
...
// bound bodyrate into a maximum change size
...
// 返回加速度对应的油门
// 若为非精确模型，则油门为线性比例，即 a / g * hover_percentage
