...
void Command_Data_t::feed(quadrotor_msgs::PositionCommandConstPtr pMsg)
{
    ...
    // SUPER publishes yaw/yaw_dot directly instead of EGO's yaw direction
    // vectors.  Reconstruct the equivalent horizontal unit vector and its
    // derivative for px4ctrl's differential-flatness implementation.
    dir = Eigen::Vector3d(std::cos(yaw), std::sin(yaw), 0.0);
    ddir = yaw_rate * Eigen::Vector3d(-std::sin(yaw), std::cos(yaw), 0.0);
}
