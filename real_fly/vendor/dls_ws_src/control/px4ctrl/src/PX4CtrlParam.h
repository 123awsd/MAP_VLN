...
public:
	...
	struct AutoTakeoffLand
	{
		...
		bool no_RC;
		double height;
		// Legacy common speed. takeoff_speed/land_speed fall back to this value
		// when the corresponding optional parameter is not configured.
		double speed;
		double takeoff_speed;
		double land_speed;
		double motors_speedup_time;
		double post_takeoff_delay;
		double land_detect_position_error;
		double land_detect_velocity_threshold;
		double land_detect_keep_time;
		double disarm_retry_interval;
	};
