...
void PX4CtrlFSM::process()
{
	...
	case AUTO_TAKEOFF:
	{
		if ((now_time - takeoff_land.toggle_takeoff_land_time).toSec() <
			param.takeoff_land.motors_speedup_time) // Ramp motors before liftoff.
		{
		...
		else if (odom_data.p(2) >= (takeoff_land.start_pose(2) + param.takeoff_land.height)) // reach the desired height
		{
			...
			takeoff_land.delay_trigger.second =
				now_time + ros::Duration(param.takeoff_land.post_takeoff_delay);
		}
		else
		{
			des = get_takeoff_land_des(param.takeoff_land.takeoff_speed);
		}
	...
	case AUTO_LAND:
	{
		...
		{
			des = get_takeoff_land_des(-param.takeoff_land.land_speed);
		}
		else
		{
			...
			if (print_once_flag)
			{
				ROS_INFO("\033[32m[px4ctrl] Touchdown detected; wait for PX4 ON_GROUND "
						 "before disarming.\033[32m");
			...
			if (extended_state_data.current_extended_state.landed_state == mavros_msgs::ExtendedState::LANDED_STATE_ON_GROUND) // PX4 allows disarm after this
			{
				...
				if (now_time.toSec() - last_trial_time > param.takeoff_land.disarm_retry_interval)
				{
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	else
	{
		const bool C12_satisfy =
			(des.p(2) - odom.p(2)) < -param.takeoff_land.land_detect_position_error &&
			odom.v.norm() < param.takeoff_land.land_detect_velocity_threshold;
		...
		else if (C12_satisfy && is_last_C12_satisfy)
		{
			if ((ros::Time::now() - time_C12_reached).toSec() >
				param.takeoff_land.land_detect_keep_time) // Constraint 3 reached
			{
				takeoff_land.landed = true;
			}
		}
...
Desired_State_t PX4CtrlFSM::get_hover_des()
{
	...
	des.yaw_rate = 0.0;
	des.dir = Eigen::Vector3d(cos(des.yaw),sin(des.yaw),0);
	des.ddir = Eigen::Vector3d::Zero();
...
Desired_State_t PX4CtrlFSM::get_rotor_speed_up_des(const ros::Time now)
{
	...
	double des_a_z =
		exp((delta_t - param.takeoff_land.motors_speedup_time) * 6.0) * 7.0 - 7.0;
	// Parameters 6.0 and 7.0 shape only the motor ramp curve.
	if (des_a_z > 0.1)
	{
...
Desired_State_t PX4CtrlFSM::get_takeoff_land_des(const double speed)
{
	...
	double delta_t = (now - takeoff_land.toggle_takeoff_land_time).toSec() -
		(speed > 0 ? param.takeoff_land.motors_speedup_time : 0); // speed > 0 means takeoff
