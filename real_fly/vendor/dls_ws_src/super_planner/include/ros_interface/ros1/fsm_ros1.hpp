...
namespace fsm {
    class FsmRos1 : public Fsm {
        ros::NodeHandle nh_;
        ros::Subscriber goal_sub_, pause_sub_;
    ...
    public:
        ...
        void pauseCallback(const std_msgs::BoolConstPtr &msg) {
            if (externally_paused_ == msg->data) {
                return;
            }
            externally_paused_ = msg->data;
            if (externally_paused_) {
                cout << YELLOW << " -- [Fsm] Pause planning and command output for preset smooth trajectory."
                     << RESET << endl;
            } else {
                cout << GREEN << " -- [Fsm] Resume SUPER planning after preset smooth trajectory."
                     << RESET << endl;
            }
        }
        ...
        void init(const ros::NodeHandle &nh, const std::string &cfg_path) {
            ...
            pause_sub_ = nh_.subscribe(cfg_.pause_topic, 1, &FsmRos1::pauseCallback, this);
