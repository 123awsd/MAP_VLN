...
//            cout << "bd  * bh: " << (bd * bh).transpose().maxCoeff() << endl;
            cout << YELLOW << " -- [CIRI_DIAG] INIT_ERROR: seed endpoint outside boundary, max_violation="
                 << std::max((bd * ah).maxCoeff(), (bd * bh).maxCoeff())
...
//                std::cout << " -- [CIRI] hPoly: " << hPoly << std::endl;
                cout << YELLOW << " -- [CIRI_DIAG] failed: generated half-space matrix contains NaN, loop="
                     << loop << " planes=" << hPoly.rows() << RESET << endl;
                return FAILED;
            }

            if (!MVIE::maxVolInsEllipsoid(hPoly, E)) {
                cout << YELLOW << " -- [CIRI_DIAG] failed: MVIE::maxVolInsEllipsoid returned false, loop="
                     << loop << " planes=" << hPoly.rows() << RESET << endl;
                return FAILED;
            }
        }
        ...
        if (std::isnan(hPoly.sum())) {
            ...
            ros_ptr_->vizCiriSeedLine(a, b,robot_r_);
            ros_ptr_->vizCiriEllipsoid(E);
            cout << YELLOW << " -- [CIRI_DIAG] failed: final half-space matrix contains NaN, planes="
                 << hPoly.rows() << RESET << endl;
            return FAILED;
        }
        Vec3f inner;
        if (!geometry_utils::findInterior(hPoly, inner)) {
            cout<<RED<<" -- [CIRI] The polytope is empty."<<RESET<<endl;
            cout << YELLOW << " -- [CIRI_DIAG] failed: findInterior returned false, planes="
                 << hPoly.rows() << " seed_start=" << a.transpose()
                 << " seed_end=" << b.transpose() << RESET << endl;
            ros_ptr_->vizCiriSeedLine(a, b,robot_r_);
            ros_ptr_->vizCiriEllipsoid(E);
            return FAILED;
        }
