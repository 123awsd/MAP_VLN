...
namespace path_search {
    ...
    RET_CODE Astar::pointToPointPathSearch(const rog_map::Vec3f &start_pt, const rog_map::Vec3f &end_pt,
        ...
        out_path.clear();
        // Path-search termination must not depend on ROS simulation time.  In
        // offline preview (and on a vehicle before a /clock source exists)
        // ros::Time can remain constant, turning a finite A* timeout into an
        // unbounded search as soon as an added local obstacle blocks a route.
        const auto wall_time_1 = std::chrono::steady_clock::now();
        ++rounds_;
        ...
        while (!open_set.empty()) {
            ...
            rog_map::Vec3f currentPos;
            globalIndexToPos(current->id_g, currentPos);
                ...
                for (int dy = -1; dy <= 1; dy++)
                    for (int dz = -1; dz <= 1; dz++) {
                        ...
                        if (transition_validator_ &&
                            !transition_validator_(currentPos, neighborPos)) {
                            continue;
                        }
            ...
            const double elapsed = std::chrono::duration<double>(
                std::chrono::steady_clock::now() - wall_time_1).count();
            if (!cfg_.visual_process && elapsed > time_out) {
