...
namespace fsm {
    ...
    void Fsm::callReplanOnce() {
        ...
        const bool is_new_goal_replan = gi_.new_goal;
        RET_CODE ret_code = planner_ptr_->ReplanOnce(gi_.goal_p, gi_.goal_yaw, gi_.new_goal);

        // Routine successful replans run at fsm/replan_rate and are intentionally
        // silent. Keep event-driven messages so goals, warnings and failures stay
        // visible in the console.
        if ((ret_code == SUCCESS || ret_code == FINISH) && is_new_goal_replan) {
            fmt::print(fg(fmt::color::green),
                       " -- [Fsm] New-goal replan succeeded: target=[{:.3f} {:.3f} {:.3f}].\n",
                       gi_.goal_p.x(), gi_.goal_p.y(), gi_.goal_p.z());
        } else if (ret_code == NEW_TRAJ) {
            cout << YELLOW << " -- [Fsm] Replan requested full trajectory regeneration." << RESET << endl;
        } else if (ret_code == EMER) {
            cout << RED << " -- [Fsm] Replan requested emergency stop." << RESET << endl;
        }
    ...
    void Fsm::callMainFsmOnce() {
        ...
        static double last_wait_print_t = -5.0;
        ...
        if (cur_t - last_print_t > 1.0) {
            ...
            if (!started_) {
                if (cur_t - last_wait_print_t > 5.0) {
                    last_wait_print_t = cur_t;
        ...
        switch (machine_state_) {
            ...
            case GENERATE_TRAJ: {
                ...
                if (retcode == SUCCESS || retcode == FINISH) {
                    ...
                    fmt::print(fg(fmt::color::green),
                               " -- [Fsm] PlanFromRest succeeded: target=[{:.3f} {:.3f} {:.3f}].\n",
                               gi_.goal_p.x(), gi_.goal_p.y(), gi_.goal_p.z());
    ...
    void Fsm::setGoalPosiAndYaw(const Vec3f &p, const Quatf &q) {
        ...
        if (cfg_.click_yaw_en) {
            if (isnan(q.w()) || isnan(q.x()) || isnan(q.y()) || isnan(q.z())) {
                gi_.goal_yaw = NAN;
                fmt::print(fg(fmt::color::green),
                           " -- [Fsm] Accepted goal: position=[{:.3f} {:.3f} {:.3f}], yaw=auto.\n",
                           gi_.goal_p.x(), gi_.goal_p.y(), gi_.goal_p.z());
            } else {
                gi_.goal_yaw = geometry_utils::get_yaw_from_quaternion(q);
                fmt::print(fg(fmt::color::green),
                           " -- [Fsm] Accepted goal: position=[{:.3f} {:.3f} {:.3f}], yaw={:.1f} deg.\n",
                           gi_.goal_p.x(), gi_.goal_p.y(), gi_.goal_p.z(),
                           gi_.goal_yaw * 180.0 / M_PI);
            }
