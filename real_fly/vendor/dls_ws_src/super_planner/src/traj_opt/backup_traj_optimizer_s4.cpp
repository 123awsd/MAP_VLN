...
double BackupTrajOpt::optimize(Trajectory &traj, const double &relCostTol) {
    ...
    const double pos_limit = 0.2;
    const double vel_limit = cfg_.max_vel * cfg_.penna_margin;
    const double acc_limit = cfg_.max_acc * cfg_.penna_margin;
    const double omg_limit = cfg_.max_omg * cfg_.penna_margin;
    const double thr_limit = cfg_.max_acc * cfg_.penna_margin;
    const bool pos_violation = cfg_.penna_pos > 0 && opt_vars.penalty_log(1) > pos_limit;
    const bool vel_violation = cfg_.penna_vel > 0 && opt_vars.penalty_log(2) > vel_limit;
    const bool acc_violation = cfg_.penna_acc > 0 && opt_vars.penalty_log(3) > acc_limit;
    const bool omg_violation = cfg_.penna_omg > 0 && opt_vars.penalty_log(6) > omg_limit;
    const bool thr_violation = cfg_.penna_thr > 0 && opt_vars.penalty_log(7) > thr_limit;
    if (pos_violation || vel_violation || acc_violation || omg_violation || thr_violation) {
        ret = -1;
        if (cfg_.print_optimizer_log) {
            cout << " -- [BaclOpt] Opt finish, with iter num: " << opt_vars.iter_num << "\n";
            cout << "\tEnergy: " << opt_vars.penalty_log(0) << endl;
        ...
        ros_ptr_->warn(
            " -- [BackOpt] Constraint residual too large: "
            "pos={:.6f}/{} ({}), vel={:.6f}/{:.6f} ({}), "
            "acc={:.6f}/{:.6f} ({}), omg={:.6f}/{:.6f} ({}), "
            "thr={:.6f}/{:.6f} ({}).",
            opt_vars.penalty_log(1), pos_limit, pos_violation ? "FAIL" : "ok",
            opt_vars.penalty_log(2), vel_limit, vel_violation ? "FAIL" : "ok",
            opt_vars.penalty_log(3), acc_limit, acc_violation ? "FAIL" : "ok",
            opt_vars.penalty_log(6), omg_limit, omg_violation ? "FAIL" : "off/ok",
            opt_vars.penalty_log(7), thr_limit, thr_violation ? "FAIL" : "off/ok");
    }
