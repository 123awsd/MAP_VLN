...
//    cout << " -- [ExpOpt] waypoint_attractor_dead_d: " << opt_vars.waypoint_attractor_dead_d.transpose() << endl;
    ...
    // The line search can reach its retry limit after it has already found a
    // finite, constraint-satisfying iterate (this is common for short,
    // single-piece take-off trajectories).  Do not discard that usable
    // iterate solely because the next Wolfe step cannot improve it.  Invalid
    // numerical states and constraint violations are still rejected above.
    if (ret == lbfgs::LBFGSERR_MAXIMUMLINESEARCH &&
        std::isfinite(minCostFunctional) && x.allFinite() &&
        opt_vars.times.allFinite() && opt_vars.penalty_log.allFinite()) {
        ros_ptr_->warn(" -- [ExpOpt] Line search limit reached; accept finite feasible iterate.");
        ret = 0;
    }
