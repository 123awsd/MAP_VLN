...
MODEL = "qwen3.7-plus"
PROMPT_VERSION = "stage2-task-graph-v15-generic-recovery-single-task"
...
必须显式判断用户意图：“看看/确认某处有没有”是 verify_presence；若没有后续动作则使用 report_absent，若用户明确指定具体备用地点、备用目标或不同后续动作，则源任务使用 verify_presence + explicit_branch，并建立独立备用 task 和 conditional_rules。“帮我找/一定要找到，应该在某处，没有就继续找/那里没有的话就继续帮我找”是单个 locate_target + semantic_recovery 任务；若“找”任务明确指定具体的不同后续动作，也可使用 locate_target + explicit_branch。“其他可能位置”“其他地方”“家里别处”“继续寻找”等模糊范围不是具体备用地点：必须保留为一个 scoped locate_target + semantic_recovery 任务，禁止预先创建无 room/floor/reference 的“全屋搜索”备用任务，也禁止直接读取全局历史目标位置绕过失败后的恢复推理。单独出现“如果/没有的话”不等于条件分支，必须根据后续是否包含具体备用地点、目标或动作判断。条件分支源任务必须 mode=fixed、on_exhaustion=finish；自主语义恢复不能与 explicit_branch 同时出现。execute_action 只用于 deliver/approach 等非视觉动作。intent.not_found_policy 必须与 search_policy.on_exhaustion 一致：semantic_recovery 对应 qwen_semantic_recovery，其余对应 finish。
